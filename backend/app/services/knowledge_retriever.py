...
class KnowledgeRetriever:
    ...
    def _get_entity(self, intent: IntentResult, *keys: str) -> Optional[str]:
        """从 intent.entities 中按多个候选键名取值，返回第一个非空实体。"""
    ...
    def _query_top_museum_by_type(self, object_id: Optional[str], intent: IntentResult) -> Optional[RetrievalResult]:
        ...
        """
        artifact_type = self._get_entity(intent, "artifact_type", "type", "artifactType")
    ...
    def _query_museum_city(self, object_id: Optional[str], intent: IntentResult) -> Optional[RetrievalResult]:
        ...
        if hasattr(intent, 'entities') and intent.entities:
            museum_name = self._get_entity(intent, "museum", "museum_name", "museumName")
            if museum_name:
                cypher = """
